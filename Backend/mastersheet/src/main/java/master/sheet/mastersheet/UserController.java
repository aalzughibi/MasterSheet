...
public class UserController {
    ...
    public static boolean InsertUser(User u) {
        try {
            ...
            String sql = "INSERT INTO " + userTable
                    + " (username,password,email,first_name,last_name,role,display_name,uid,BirthDate,first_time) "
                    + String.format("VALUES ('%s','%s','%s','%s','%s',%s,'%s','%s','%s',%s)", u.getUsername(),
                            generatePassword(10), u.getEmail(), u.getFirst_name(), u.getLast_name(), u.getRole(),
                            u.getDisplay_name(), RandomStringUtils.randomAlphanumeric(40), u.getBirthDate(), 0);
    ...
    public ResponseEntity<User> getUserByUid(@RequestHeader("Authorization") String header_auth,
        ...
        try {
            if (Auth.validJWT(header_auth)) {// start//end
                ...
                if (isAdmin(String.valueOf(jo.get("userId")))) {
                    Class.forName("org.mariadb.jdbc.Driver");
                    Connection con = DriverManager.getConnection("jdbc:mariadb://localhost:" + port + "/" + database,
                            username, password);
                    Statement stmt = con.createStatement();
    ...
    public ResponseEntity<Map<String, Object>> updateUser(@RequestHeader("Authorization") String header_auth,
    ...
    public ResponseEntity<Map<String, Object>> updateDisplayName(@RequestHeader("Authorization") String header_auth,
            @RequestBody User user, @PathVariable String userId) {
        try {
            if (Auth.validJWT(header_auth)) {
                JSONObject jo = Auth.convert_JsonString_To_Json(Auth.getJWTToken(header_auth)[1]);
    ...
    private static boolean updateUser(User user) {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mariadb://localhost:" + port + "/" + database, username,
                    password);
            Statement stmt = con.createStatement();
            String sql = "UPDATE " + userTable + " SET " + "first_name='" + user.getFirst_name() + "', last_name='"
                    + user.getLast_name() + "', display_name='" + user.getDisplay_name() + "', role=" + user.getRole()
    ...
    private static boolean updateDisplayName(User user) {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mariadb://localhost:" + port + "/" + database, username,
                    password);
            Statement stmt = con.createStatement();
            String sql = "UPDATE " + userTable + " SET " + "display_name='" + user.getDisplay_name() + "' WHERE uid='"
                    + user.getUid() + "'";
            stmt.executeUpdate(sql);
            ...
            if (Auth.validJWT(header_auth)) {// start//end
                ...
                if (isAdmin(String.valueOf(jo.get("userId")))) {
                    ...
                    if (Validate.isValidUsername(u.getUsername())
                            ...
                            && ((u.getDisplay_name().length() >= 2) && (u.getDisplay_name().length() <= 20))) {// start
                                                                                                               // //end
